...
public class DataInitializationController {
	...
	@Inject
	public DataInitializationController(CompanyEntityRepository companyEntityRepository,
			ShiftRepository shiftRepository, CalendarRepository calendarRepository,
			ClockingInfoRepository clockingInfoRepository, ReminderListRepository reminderListRepository,
	...
	@GetMapping(path = "/data/initialization")
	public String dataInitialization(Model model) {
		...
		RegularEvent regularEvent = new RegularEvent(LocalDateTime.now().withDayOfMonth(1).withHour(8),
				LocalDateTime.now().withDayOfMonth(1).withHour(17));
